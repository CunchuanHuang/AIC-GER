...
class Exp_loss_global(torch.nn.Module):
    ...
    def forward(self, x):
        """
        Args:
            x: Input image tensor. Expected shape: (H, W, 3) or (B, H, W, 3).
               In your 3DGS context, it is likely (H, W, 3).
        """
        if x.dim() == 3:
            lum = torch.mean(x, -1, keepdim=True)
            lum = lum.unsqueeze(0)
            lum = lum.permute(0, 3, 1, 2)         
        elif x.dim() == 4:
            lum = torch.mean(x, -1, keepdim=True)
            lum = lum.permute(0, 3, 1, 2)
        else:
...
def train(config_path, device="cuda"):
    ...
    for step in pbar:
        ...
        gt_image = gamma_augment(gt_image.to(device))
        # ===================================================================
        ...
        loss_control = Exp_loss_global(mean_val=0.45)(rendered)
        # loss = (1.0 - cfg.LAMBDA_SSIM) * l1_loss + cfg.LAMBDA_SSIM * (1.0 - ssim_val) + 0.5 * loss_control
        loss = (1.0 - cfg.LAMBDA_SSIM) * l1_loss + cfg.LAMBDA_SSIM * (1.0 - ssim_val) + loss_control
